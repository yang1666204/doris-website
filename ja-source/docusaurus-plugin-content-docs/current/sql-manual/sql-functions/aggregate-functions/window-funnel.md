...
| `<timestamp>` | timestamp はDATETIME型の列を指定し、スライディング時間窓がその列に対して動作します |
...
**Mode**
    ...
    - `fixed`：チェーンは指定された順序で進む必要があり、中間ステップをスキップできません。直前の条件がマッチする前に後の条件にマッチするイベントが現れた場合、チェーンは停止します。いずれの条件にもマッチしないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D']に対して、`A-B-D-C`は`A-B`とレベル`2`を返します。`A-B-X-C-D`（`X`はいずれの条件にもマッチしない）の場合、Dorisは`A-B-C-D`を返します。
...
`default`モードを使用して、時間窓を`5`分として、異なる`user_id`に対応する連続イベントの最大数を求める：
...
+---------+-------+
```
`user_id=100123`の場合、`payment`イベントが発生した時刻が時間ウィンドウを超過しているため、マッチしたイベントチェーンは`login-visit-order`です。
...
+---------+-------+
```
`user_id=100123`の場合、`login2`はファネル内のどの条件にも一致しないため、`fixed`チェーンを中断しません。マッチしたイベントチェーンは`login-visit-order-payment`です。
