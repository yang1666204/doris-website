...
| `<timestamp>` | timestampはDATETIME型の列を指定し、スライディング時間ウィンドウがその上で動作します |
...
**Mode**
    ...
    - `fixed`：チェーンは指定された順序で進行する必要があり、中間ステップをスキップできません。後の条件にマッチするイベントが、その直前の条件がマッチする前に出現した場合、チェーンは停止します。いずれの条件にもマッチしないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D']において、`A-B-D-C`は`A-B`を返し、レベルは`2`です；`A-B-X-C-D`（`X`はいずれの条件にもマッチしない）の場合、Dorisは`A-B-C-D`を返します。
...
`deduplication`モードを使用して、異なるuser_idに対応する連続イベントの最大数を、1時間の時間ウィンドウで見つけます：
...
+---------+-------+
```
`user_id=100123`の場合、`login2`はファンネル内のどの条件にも一致しないため、`fixed`チェーンを破ることはありません。一致したイベントチェーンは`login-visit-order-payment`です。
