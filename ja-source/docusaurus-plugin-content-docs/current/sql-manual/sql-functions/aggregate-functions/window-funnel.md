...
| `<timestamp>` | timestampはDATETIME型の列を指定し、スライディング時間ウィンドウはこの列に対して動作します |
| `<event_n>` | event_nはeventID = 1004のようなboolean式です |

**Mode**

    - `default`: 標準的なファネル計算。Dorisは時間ウィンドウ内で指定された順序に一致する最長のイベントチェーンを検索します。どの条件にも一致しないイベントは無視されます。

    - `deduplication`: `default`ベースですが、現在のチェーンで既に一致したイベントは再度出現できません。例えば、条件リストが[event1='A', event2='B', event3='C', event4='D']で、元のイベントチェーンが`A-B-C-B-D`の場合、2番目の`B`がチェーンを破るため、一致するイベントチェーンは`A-B-C`となり、最大レベルは`3`になります。

    - `fixed`: チェーンは指定された順序で進行する必要があり、中間ステップをスキップできません。直前の前提条件が一致する前に、後の条件に一致するイベントが現れた場合、チェーンが停止します。どの条件にも一致しないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D']で、`A-B-D-C`は`A-B`とレベル`2`を返します。`A-B-X-C-D`（`X`はどの条件にも一致しない）の場合、Dorisは`A-B-C-D`を返します。

    - `increase`: `default`ベースですが、一致したイベントは厳密に増加するタイムスタンプを持つ必要があります。2つの一致したイベントが同じタイムスタンプを持つ場合、後のイベントはチェーンを進めることができません。
...
+---------+-------+
```
`user_id=100123`の場合、`login2`はファネル内のどの条件にも一致しないため、`fixed`チェーンを破ることはありません。マッチしたイベントチェーンは`login-visit-order-payment`です。
