...
| パラメータ | 説明 |
| -- | -- |
| `<window>` | window は秒単位の時間窓の長さです |
| `<mode>` | 合計4つのモードがあります：`default`、`deduplication`、`fixed`、`increase`。詳細は以下の**Mode**を参照してください。 |
...
**Mode**
    ...
    - `fixed`: チェーンは指定された順序で進む必要があり、中間ステップをスキップできません。直前の条件がマッチする前に後の条件にマッチするイベントが現れた場合、チェーンは停止します。どの条件にも一致しないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D']で、`A-B-D-C`は`A-B`とレベル`2`を返します。`A-B-X-C-D`（`X`はどの条件にもマッチしない）では、Dorisは`A-B-C-D`を返します。
...
+---------+-------+
```
`user_id=100123`の場合、`payment`イベントが発生した時刻が時間ウィンドウを超えているため、マッチした イベントチェーンは`login-visit-order`です。
...
`fixed`モードを使用して、異なる`user_id`に対応する連続イベントの最大数を求めます。時間窓は`1`時間です：
...
+---------+-------+
```
`user_id=100123`の場合、`login2`はファネル内のどの条件にも一致しないため、`fixed`チェーンを破綻させません。一致するイベントチェーンは`login-visit-order-payment`です。
...
+---------+-------+
```
`user_id=100123`の場合、`payment`イベントのタイムスタンプと`order`イベントのタイムスタンプが同じ秒で発生し、増加していないため、マッチしたイベントチェーンは`login-visit-order`です。
