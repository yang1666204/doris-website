...
- データに完了度の異なる複数のイベントチェーンがある場合、関数は最長チェーンのサイズのみを出力します。
...
| `<timestamp>` | timestampはDATETIME型の列を指定し、スライディング時間窓はその列に対して動作します |
...
**Mode**
    ...
    - `fixed`: チェーンは指定された順序で進む必要があり、中間ステップをスキップできません。直前の条件がマッチする前に後の条件にマッチするイベントが現れると、チェーンが停止します。どの条件にも一致しないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D']で、`A-B-D-C`は`A-B`とlevel `2`を返します；`A-B-X-C-D`（`X`はどの条件にも一致しない）では、Dorisは`A-B-C-D`を返します。
...
+---------+-------+
```
`user_id=100123`の場合、`payment`イベントが発生した時刻がタイムウィンドウを超えるため、マッチしたイベントチェーンは`login-visit-order`です。
