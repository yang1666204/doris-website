...
| パラメータ | 説明 |
| -- | -- |
| `<window>` | window は時間ウィンドウの長さを秒単位で指定 |
...
**Mode**
    ...
    - `default`：標準的なファネル計算。Doris は時間ウィンドウ内で指定された順序に一致する最長のイベントチェーンを検索します。どの条件にも一致しないイベントは無視されます。
    ...
    - `fixed`：チェーンは指定された順序で進む必要があり、中間ステップをスキップできません。直前の条件がマッチする前に、後の条件にマッチするイベントが現れた場合、チェーンは停止します。どの条件にも一致しないイベントは無視され、チェーンを破りません。例えば、[event1='A', event2='B', event3='C', event4='D'] で `A-B-D-C` は `A-B` とlevel `2` を返し、`A-B-X-C-D`（`X` はどの条件にもマッチしない）では、Doris は `A-B-C-D` を返します。
...
`deduplication`モードを使用して、時間窓1時間で、異なるuser_idに対応する連続イベントの最大数を調べます：
...
`fixed`モードを使用して、異なる`user_id`に対応する連続イベントの最大数を、`1`時間のタイムウィンドウで見つけます：
