...
- DLF 1.0バージョンの場合、DorisはDLFのHive Metastore互換インターフェース経由でDLFにアクセスします。Paimon カタログとHive カタログをサポートします。
- DLFバージョン2.5以降の場合、DorisはDLFのRestインターフェース経由でDLFにアクセスします。Paimon カタログのみサポートします。
...
| `dlf.catalog_id` | `dlf.catalog.id` | カタログ ID。メタデータカタログを指定するために使用。設定されていない場合、デフォルトカタログが使用されます。 | なし | いいえ |
| `warehouse` | - | Warehouseのストレージパス。Paimon カタログでのみ必要。オブジェクトストレージパスは`/`で終わる必要があることに注意。 | なし | いいえ |
...
| `warehouse` | - | Warehouse名。注意: 接続するカタログの名前を直接入力し、Paimonテーブルのストレージパスではない | なし | はい |
...
DLF Rest カタログでは、ストレージサービス（OSS）のEndpointとRegion情報を提供する必要はありません。DorisはDLF Rest カタログのVended CredentialでOSSにアクセスするための一時的な認証情報を取得します。
