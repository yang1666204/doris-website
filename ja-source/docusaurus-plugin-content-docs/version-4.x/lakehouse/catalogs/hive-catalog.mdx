...
Hive に加えて、他の多くのシステムが Hive Metastore を使用してメタデータを保存しています。そのため、Hive カタログ を通じて、Hive テーブルだけでなく、Iceberg や Hudi などの Hive Metastore をメタデータストレージとして使用する他のテーブル形式にもアクセスできます。
