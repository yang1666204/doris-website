---
{
  ...
  "language": "ja",
  "description": "この記事では、Apache DorisとAlibaba Cloud DLF（Data Lake Formation）Rest カタログを統合して、Paimonテーブルデータへのシームレスなアクセスと分析を行う方法を説明します。カタログの作成、データのクエリ、および増分読み取りのガイドが含まれています。"
}
---
...
DLFはバージョン2.5からPaimon Rest カタログをサポートしています。Dorisはバージョン3.0.3/3.1.0からDLF 2.5+ Paimon Rest カタログとの統合をサポートし、DLFへのシームレスな接続を可能にしてPaimonテーブルデータにアクセスし分析できます。この記事では、Apache DorisをDLF 2.5+と接続し、Paimonテーブルデータにアクセスする方法を説明します。
