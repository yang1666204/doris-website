---
{
  ...
  "language": "ja",
  "description": "AWS S3 Tablesは、Apache Icebergテーブルフォーマット標準と互換性のある読み取りおよび書き込みインターフェースを提供する特別なタイプのS3 バケットです。"
}
---
[AWS S3 Tables](https://aws.amazon.com/s3/features/tables/)は、Apache Icebergテーブル形式標準と互換性のある読み取りおよび書き込みインターフェースを提供する特別なタイプのS3 バケットで、Amazon S3上に構築されており、S3自体と同じ耐久性、可用性、スケーラビリティ、およびパフォーマンス特性を提供します。さらに、S3 Tablesは以下の機能を提供します：
...
S3 Tablesのリリースは、レイクハウスアーキテクチャをさらに簡素化し、クラウドネイティブなレイクウェアハウスシステムに更なる可能性をもたらします。これには、コールド・ホット分離、データアーカイブ、データバックアップ、コンピュート・ストレージ分離アーキテクチャが含まれ、これらすべてがS3 Tablesベースの全く新しいアーキテクチャに進化する可能性があります。
...
ここでは、doris-s3-table-bucketという名前のtable バケットを作成します。作成後、ARNで表されるtable バケットを取得します。

![AWS S3 table バケット Create](/images/レイクハウス/s3-table-bucket-create.png)

### 02 Iceberg カタログの作成

- AWS S3 table Rest カタログを使用して`s3 tables`に接続する
